<template>
    <div class="relative">
      ...
      <div class="grid items-center justify-items-center cursor-pointer" @click="toggleMenu">
        <img src="@/assets/images/user_icon_logged.png" alt="Login" 
          class="w-6 h-6 hover:p-[0.05rem]">
        <label class="font-[0.8rem] mt-[.2rem]">{{ user.name }}</label>
      </div>
  
      <!-- Dropdown Menu -->
      <div v-if="showMenu" class="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg">
        <ul>
          <li>
            <router-link 
              to="/configuracoes" 
