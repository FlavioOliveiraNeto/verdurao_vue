<template>
    <div class="h-max w-182 mx-auto my-auto items-center gap-x-10 rounded-xl p-6">
      <div :class="type === 'admin' ? 'bg-sky-900' : 'bg-green-800'" class="grid rounded-lg shadow-lg py-5 px-7 gap-y-2">
        ...
        <p class="text-white text-xl font-bold w-fit place-self-center mb-[1rem]">Login de {{ type }}</p>
        <div class="grid gap-y-2 mb-[1rem]">
          ...
          <div class="flex justify-between">
            <button @click="showRegister" class="w-fit mt-2 text-white hover:underline cursor-pointer text-start">Criar cadastro</button>
            <button @click="forgotPassword" class="w-fit mt-2 text-white hover:underline cursor-pointer text-start">Esqueci minha senha</button>
          </div>
        </div>
        <button @click="login" :class="type === 'admin' ? 'bg-blue-500 hover:bg-blue-600' : ' bg-green-500 hover:bg-green-600'" 
