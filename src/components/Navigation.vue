<template>
  <nav class="flex justify-between items-center py-4 border-b">
    ...
    <!-- Search Bar -->
    <div class="flex mt-4 w-[40%] font-medium">
      <input type="text" placeholder="Procure por produtos ou marcas" class="border p-2 w-full rounded-l font-medium" />
      <button class="bg-black text-white px-4 py-2 rounded-r hover:cursor-pointer hover:bg-gray-800">Buscar</button>
    </div>
    ...
    <div class="flex gap-6">
      ...
      <div class="relative">
        <router-link 
          ...
          @click="toggleCart"
        >
          <img src="@/assets/images/shop_icon.png" alt="Carrinho" class="w-6 h-6 cursor-pointer hover:p-[0.05rem]">
        </router-link>
        <img 
          v-else 
      ...
      <router-link v-else to="/login" class="text-gray-700 hover:text-black">
        <img src="@/assets/images/user_icon.png" alt="Login" class="w-6 h-6 cursor-pointer hover:p-[0.05rem]">
      </router-link>
    </div>
  </nav>
</template>
